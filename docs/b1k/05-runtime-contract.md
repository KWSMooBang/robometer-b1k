...
`SUBOPTIMAL` 선호 전략은 truncated 네거티브 덕분에 **이미 작동 중**이다(초안에는 미작동이라 적었으나
오류). 롤아웃 실패 데이터가 들어오면 그 전략이 "중간에 끊긴 클립" 대신 **진짜 실패 궤적**을 뽑게 되어
질이 달라진다 — 지금 학습되는 것은 "아직 안 끝남"이고, 그때 학습되는 것이 "잘못됨"이다.
FSM의 recovery 트리거는 후자를 필요로 하므로, 현재 데이터로는 검증되지 않은 상태다.
